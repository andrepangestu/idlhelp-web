...
The VPS serves `main` at `https://idlhelp.com`; the GitHub build and VPS deployment jobs have passed. DNS points to `206.189.84.142`, and Let's Encrypt HTTPS is active for both apex and `www`. Certificate renewal passed a dry run. The production secret is configured separately in GitHub.

The existing Netlify project publishes `development` at `https://idlhelp.netlify.app`. Its production branch is set under **Project configuration → Build & deploy → Continuous deployment → Branches and deploy contexts**, with builds active. Set branch deploys to **None** if no other branch needs a Netlify URL. Netlify publishes directly from Git without any GitHub Netlify token or Project ID secret. Keep `idlhelp.com` off the Netlify custom domains because DNS directs it to the VPS. `netlify.toml` skips Netlify builds on `main` and sends `X-Robots-Tag: noindex, nofollow` on the development site.
